	...
	public void upateFriend(Message m)
	{
		...
		if(m.getMesType().equals(MessageType.message_exit)){
			for(int i=0;i<onLineFriend.length;i++)
			{
				...
				String friend;
				QqChat temp;
				if((temp=ManageQqChat.getQqChat(owner+" "+(friend=jb1s[Integer.parseInt(onLineFriend[i])-1].getText())))!=null){
					temp.setTitle((owner+" 正在和 "+friend+"（离线中）"+" 聊天"));
				}
			}
		}
		else{
			if(login!=0){
				try {
					AudioInputStream ais = AudioSystem.getAudioInputStream(getClass().getResource("login.wav"));
...
//				if(!jb1s[Integer.parseInt(onLineFriend[i])-1].getText().equals(owner)){
//					try {
//						AudioInputStream ais = AudioSystem.getAudioInputStream(getClass().getResource("login.wav"));
//						Clip  clip = AudioSystem.getClip();
//						clip.open(ais);
...
//				}
				...
				QqChat temp;
				if((temp=ManageQqChat.getQqChat(owner+" "+(friend=jb1s[Integer.parseInt(onLineFriend[i])-1].getText())))!=null){
					temp.setTitle((owner+" 正在和 "+friend+"（在线中）"+" 聊天"));
				}
			}
		}
	}
	
	public QqFriendList(final String ownerId)
	{	
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		{	
			...
			if(friendNo.equals(owner)){
				JOptionPane.showMessageDialog(null, "和自己聊天没多大意思");
			}
			//说明已经打开了，让窗口出现在最前端
			QqChat qct;
